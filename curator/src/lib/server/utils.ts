...
import type { Note, Resource, PError } from '$lib/types';
 ...
 */
export async function createThumbnail(pb: PocketBase, recordID: string, resources: Resource[]) {
 ...
 */
export async function addThumbnailToRecord(pb: PocketBase, recordID: string, thumbURL: string) {
 ...
 * Clear attachments
 */
export async function deleteAllAttachments(pb: PocketBase, recordID: string) {
	const { data: record, error } = await tryCatch(
		pb.collection(notesCollection).update(recordID, {
			attachments: []
		})
 ...
 */
export async function addAsOnlyFileToRecord(pb: PocketBase, recordID: string, file: File) {
	// upload to database
 ...
 */
async function createOneNewResource(pb: PocketBase, recordID: string, resource: Resource) {
	...
	return {
		name: resource.name,
		fileURL: await uploadFileToPocketbase(pb, recordID, attachment),
 ...
 */
export async function createNewResources(pb: PocketBase, recordID: string, records: RecordModel[]) {
 ...
 */
export async function addResourcesToRecord(
	pb: PocketBase,
	recordID: string,
